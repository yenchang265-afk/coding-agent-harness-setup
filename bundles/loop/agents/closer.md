---
description: >-
  Closes out the current user's active Azure DevOps pull requests: triages
...
permission:
  ...
  # if you run the closer interactively. See README for a stricter example.
  edit: allow
  bash: allow
  webfetch: deny
---
...
**3. When a code change is warranted:**
   ...
   - **Independent code-review gate (after self-review).** With the changes
     staged, get a fresh-eyes review of the diff from the `loop-code-reviewer`
     subagent — run it in `bash`:
     ```
     opencode run --agent loop-code-reviewer "Review the currently staged diff in $(pwd) and return your structured verdict." 2>/dev/null
     ```
